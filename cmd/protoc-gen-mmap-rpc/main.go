...
func generateService(g *protogen.GeneratedFile, service *protogen.Service) {
	...
	for _, method := range service.Methods {
		g.P("    s.RegisterHandler(_", service.GoName, "_", method.GoName, "_FullMethodName, func(ctx context.Context, in []byte, out []byte) (int, error) {")
		g.P("        return handleRequest(ctx, in, out, srv.", method.GoName, ", &", method.Input.GoIdent.GoName, "{})")
		g.P("    })")
		g.P()
	}
	g.P("}")
	...
	// Generate handleRequest helper with updated signature and buffer handling
	...
	g.P("        return 0, err")
	g.P("    }")
	g.P("    // Marshal directly into the output buffer")
	g.P("    mo := proto.MarshalOptions{}")
	g.P("    outBuf, err = mo.MarshalAppend(outBuf[:0], resp)")
	g.P("    if err != nil {")
	g.P("        return 0, err")
	g.P("    }")
	g.P("    return len(outBuf), nil")
	g.P("}")
}
